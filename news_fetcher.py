...
def get_market_news(max_per_feed=10):
    ...
    for feed_name, feed_url in NEWS_FEEDS.items():
        try:
            ...
            print(f"⚠️  Could not fetch {feed_name}: {e}")

    print(f"✅ Market news: {len(all_articles)} articles from {len(NEWS_FEEDS)} sources")
    return all_articles
...
def get_news_for_top_stocks(top_stocks_data, max_per_stock=3, progress_cb=None):
    ...
    print(f"✅ News fetched for {len(news_map)} stocks")
